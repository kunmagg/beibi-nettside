...
When exactly Henrik and Tormod are flipped and every other portrait faces forward, `assets/wordmark-language.js` switches the lettering and instruments to Bokmål. The title keeps the same width and height: shared G/U/T/N/final E letters slide and resize without fading, while the A exits and the extra T and middle E fade in from below. Adjective rows appear progressively from bottom to top, without bounce. Turning either portrait back or flipping any other portrait restores Nynorsk. Quick reversals continue from the current letter positions; reduced-motion preferences skip the animation.
